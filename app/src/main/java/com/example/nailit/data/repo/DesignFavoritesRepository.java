...
public class DesignFavoritesRepository {
    ...
    public void getMyFavoriteDesigns(FavoritesListCallback callback) {
        String sub = tokenStore.getSubFromJwt();
        Log.d(TAG, "getMyFavoriteDesigns jwt_sub=" + sub);

        Consumer<String> fetch = userId -> {
            String userEq = "eq." + userId;
            Log.d(TAG, "GET user_favorite_designs user_id=" + userEq + " select=design_id");
            favoritesApi.getMyFavorites("design_id", userEq).enqueue(new Callback<List<DesignFavoriteRow>>() {
                @Override
                public void onResponse(@NonNull Call<List<DesignFavoriteRow>> call,
                    ...
                    List<DesignFavoriteRow> body = response.body();
                    Log.d(TAG, "Design favorites fetch returned count=" + body.size()
                            + " app_user_id=" + userId);
                    Set<Long> ids = new HashSet<>();
                    for (DesignFavoriteRow row : body) {
                        if (row != null && row.getDesignId() > 0) {
        ...
        if (cached != null && !cached.isEmpty()) {
            Log.d(TAG, "getMyFavoriteDesigns using cached app_user_id=" + cached);
            fetch.accept(cached);
        } else {
    ...
    private void doAddFavorite(String userId, long designId, FavoriteCallback callback) {
        ...
        Log.d(TAG, "POST user_favorite_designs body=" + body + " jwt_sub=" + tokenStore.getSubFromJwt());
    ...
    public void removeFavorite(long designId, FavoriteCallback callback) {
        Consumer<String> go = userId -> {
            String userEq = "eq." + userId;
            String designEq = "eq." + designId;
            Log.d(TAG, "DELETE user_favorite_designs user_id=" + userEq + " design_id=" + designEq
                    + " jwt_sub=" + tokenStore.getSubFromJwt());
            favoritesApi.removeFavorite(userEq, designEq).enqueue(new Callback<ResponseBody>() {
                @Override
                public void onResponse(@NonNull Call<ResponseBody> call,
                                       @NonNull Response<ResponseBody> response) {
